...
class GConv(nn.Module):
    ...
    def forward(self, u, return_kernel=False):
        ...
        m = nn.GELU()
        y_f_real = contract('bhl,chl->bchl', u_f.real, k_f.real) - contract('bhl,chl->bchl', u_f.imag, k_f.imag)
        y_f_imag = contract('bhl,chl->bchl', u_f.imag, k_f.real) + contract('bhl,chl->bchl', u_f.real, k_f.imag)
        
        y_f_real = y_f_real+contract('bhl,ch->bchl', u_f.real, self.D_real)-contract('bhl,ch->bchl', u_f.imag, self.D_imag)
        y_f_imag = y_f_imag+contract('bhl,ch->bchl', u_f.imag, self.D_real)+contract('bhl,ch->bchl', u_f.real, self.D_imag)
        
        y = torch.fft.irfft(torch.complex(m(y_f_real),m(y_f_imag)), n=2*L)[..., :L]
